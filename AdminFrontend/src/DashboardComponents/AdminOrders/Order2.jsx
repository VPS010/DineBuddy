...
const Input = ({ type = 'text', placeholder, value, onChange, className = '' }) => (
  <input
    ...
    value={value}
    onChange={onChange}
    className={`p-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500 ${className}`}
  />
);
...
const Button = ({ children, variant = 'primary', onClick, className = '' }) => {
  ...
  return (
    <button 
      className={`px-4 py-2 rounded transition-colors duration-200 ${variants[variant]} ${className}`}
      onClick={onClick}
    >
      {children}
    </button>
